...
function kkt_stuff(A,B,nx,nu,x0,N,Q,Qf,R)
    ...
    # check constraints on 5
    # A[4]*dX[4] + B[4]*dU[4] - dX[5]
    # A[5]*dX[5] + B[5]*dU[5] - dX[6]
    # mini kkt for 5
    # lilP = Q
    # lilq = Q*xtraj[5]
    # lilC = [-I;A[5]]
    # lild = [-A[4]*dX[4] - B[4]*dU[4]; -B[5]*dU[5] + dX[6]]
    #
    # @infiltrate
    # error()
    return dX, dU, λ
...
dX_kkt, dU_kkt, λ = kkt_stuff(A,B,nx,nu,x0,N,Q,Qf,R)
...
function ilqr_stuff(A,B,Nx,Nu,x0,N,Q,Qf,R)
    ...
    return dX_ilqr, dU_ilqr, q_vec, r_vec, s_vec

end

dX_ilqr, dU_ilqr,q_vec, r_vec, s_vec = ilqr_stuff(A,B,nx,nu,x0,N,Q,Qf,R)
