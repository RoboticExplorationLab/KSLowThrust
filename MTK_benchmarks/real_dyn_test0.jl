...
function fastdynamics(x)
    x_n =x[1:14]
    u = x[15:17]
    t = x[18]
    return dynamics(x_n,u,t)
end



@variables x_sym[1:18]
params = (tscale = 1e7, dscale = 2e3, uscale = 1e3)
sym_out = fastdynamics(x_sym)

sym_out2 = simplify.(sym_out)
f_expr = build_function(sym_out2,x_sym)
fast_dyn = eval(f_expr[1])




@btime fast_dyn(randn(18))

jac = simplify.(MTK.sparsejacobian(sym_out2,x_sym))
f_expr = build_function(jac,x_sym)
fast_jac = eval(f_expr[1])

@btime fast_jac(randn(18))
